...
struct BinaryTreeNode {
    int value{};
    std::unique_ptr<std::unique_ptr<TreeNode>> left{};
    std::unique_ptr<std::unique_ptr<TreeNode>> right{};
};
\end{lstlisting}
...
You can use macros to quickly print out the names value of expressions with the following macro. Note that you should always output to \texttt{std::cerr} (I think \texttt{std::clog}) as \texttt{std::cout} is used to evaluate the result.
\begin{lstlisting}
#define DS_DBG(x) std::cerr << #x << " = " << (x) << '\n';
#define DS_DBG2(x, y) std::cerr << #x << " = " << (x) << ", " << #y << " = " << (y) << '\n';

#define DS_DBG_t1(x) std::cerr << "\t" << #x << " = " << (x) << '\n';
#define DS_DBG_t2(x) std::cerr << "\t\t" << #x << " = " << (x) << '\n';
\end{lstlisting}
...
\end{lstlisting}
If the lists don't alias then this is embarassingly parallel, can just do a type of tree reduce on the individual combinations.
...
The solution to this is known Kadane's algorithm, we greedily decide in each step if we should start a ne w subarray or extend the current one. There is never a reason to backtrack.
\begin{lstlisting}
...
while(true) {
    ...
    const auto B_right = (num_B == A.size()) ? long_max : get_rotated(B, pivot_B, num_B);
...
template <typename DeadlineT, typename ProfitT>
auto collect_tasks(const std::vector<DeadlineT>& deadlines, const std::vector<ProfitT>& profits) -> TaskT<DeadLineT, ProfitT> {
...
\begin{itemize}
    ...
    \item[Precision] Measurements are "informativ".
\end{itemize}
