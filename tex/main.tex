...
\paragraph{Rotated arrays.}
If \(A\) and \(B\) are rotated-sorted, compute pivots \(p_A, p_B\) (indices of minima) and use
virtual access \(A^\ast[t] = A[(p_A+t)\bmod n]\), \(B^\ast[t] = B[(p_B+t)\bmod m]\) in the same formulas.
...
\]
are sorted. The rotation offset (which I call pivot) can be computed as follows
\begin{lstlisting}
template <typename T>
auto compute_pivot(std::span<const T> xs) -> usize {
    ...
    const auto lo = 0zu;
    const auto hi = xs.size() - 1;

    while(lo < hi) {
        const auto lo_start = lo;
        const auto hi_start = hi;
        const auto mid = std:midpoint(lo, hi);

        if(xs[mid] < xs[hi]) { // pivot is not in (mid, hi]
            hi = mid;
        } else if (xs[lo] < a[mid]) { // pivot is not in [lo, mid)
            lo = mid + 1; // voids lo == mid, hi = mid + 1 infinte loop
        } else {
            // Handling potential duplicates
            assert(xs[mid] == xs[hi]);
            --hi;
        }
        assert((lo_start < lo || hi < hi_start) && "Got stuck in compute_pivot");
    }
    return lo;
}
\end{lstlisting}
this can easily be tested for correctness as follows:
\begin{lstlisting}
template <typename T>
auto get_rotated_vector(std::span<const T> xs, long rotation) -> std::vector<T> {
    std::vector<T> out;
    out.resize(xs.size());
    for(auto i = 0zu; i < xs.size(); ++i) {
        const auto i_l = static_cast<long>(i);
        out[static_cast<usize>(i_l + rotation)] = xs[i];
    }
    return out;
}

auto test_compute_pivot() -> void {
    std::vector<int> vec{};
    vec.reserve(20);
    for(auto vec_size = 0zu; vec_size < 20; ++vec_size) 
    {
        vec.push_back(vec_size + 1);
        vec_size_l = static_cast<long>(vec)
        for(auto rotation = -vec_size_l + 1; rotation < vec_size_l - 1; ++rotation) {
            const auto vec_rotated = get_rotated_vector(vec, i);
            const auto pivot = get_pivot(vec_rotated);
            assert(pivot == rotation || pivot == rotation + vec_size_l - 1);
        }
    }
}
\end{lstlisting}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\linewidth]{MedianOfRotation.png}\\[0.3em]
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\linewidth]{MedianCircularSorted.png}\\[0.3em]
    \caption{Visualisation of Bounds}
    \label{fig:median-circular-sorted}
\end{figure}


We are interested in having \texttt{num\_left = num\_A + num\_B} elements in total where \texttt{num\_\{A, B\}} are the number of elements on the left partition.
\begin{lstlisting}
