...
\setmonofont[
  Path=../fonts/,
  UprightFont=MonaspaceKrypton-Regular.otf,
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{input_iterator.png}
        \caption*{Input}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{output_iterator.png}
        \caption*{Output}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{forward_iterator.png}
        \caption*{Forward}
    \end{minipage}
    ...
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{bidirectional_iterator.png}
        \caption*{Bidirectional}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{random_access_iterator.png}
        \caption*{Random Access}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{contiguous_iterator.png}
        \caption*{Contiguous}
    \end{minipage}
    
    \caption{Iterator Categories}
    \end{figure}
...
\newpage
\section{Computer Architecture,\\HPC (High Performanc Computing)}
...
\subsubsection{AoS vs SoA vs AoSoA}j
...
struct TransformBlock {
    std::array<f23, 8> pos_x;
    std::array<f23, 8> pos_y;
    std::array<f23, 8> pos_z;
    /*...*/
};
\end{lstlisting}
...
There are the following types of iterators:
\begin{itemize}
\item \texttt{Input Iterator}, single-pass, read-only
\item \texttt{Output Iterator}, single-pass, write-only
\item \texttt{Forward Iterator}, multi-pass, read/write, only moves forward, \texttt{std::unordered\_map} iterator
\item \texttt{Bidirectional Iterator}, multi-pass, can move forward or backward
\item \texttt{Random access Iterator}, supports jumping, index accessing, comparison
\item \texttt{Contiguous Iterator} (was added in C++20 so the Concurrency Book doesn't mention it) It's basically like a random access iterator but it also guarantees that things are laid out contiguously in memory. For example \texttt{std::vector} is contiguous while \texttt{std::deque} stores in blocks so it's only random access.
\end{itemize}
\texttt{Input Iterator}
