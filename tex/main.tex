...
\end{lstlisting}
As a sidenote, this kind of indexing is prohibitevly expensive for efficient data structures like ring buffers, there we require the size to be a multiple of 2 and we just mask it off directly, but this can't be assumed here.
...
The general case first computes how my elements we want to have to the left of the median, which is \texttt{num\_left = (A.size() + B.size() + 1) / 2}, compute bounds for what ranges in A this could be satisfied with and then doing binary search to find the desired result. First we think about what the bounds on A can be
\begin{lstlisting}
const auto num_A_min = 
const auto num_A_max = 
\end{lstlisting}
